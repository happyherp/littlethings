...
transitive_rule = IF( AND('(?a) beats (?b)', '(?b) beats (?c)'), THEN('(?a) beats (?c)') )
...
same_identity_rule = IF (OR('male (?p)', 'female (?p)'), THEN ('same-identity (?p) (?p)'))
sibling_rule = IF (AND ('parent (?p) (?x)','parent (?p) (?y)', NOT('same-identity (?x) (?y)')), THEN('sibling (?x) (?y)'))
brother_rule = IF (AND('male (?x)', 'sibling (?x) (?y)'), THEN('brother (?x) (?y)'))
sister_rule = IF (AND('female (?x)', 'sibling (?x) (?y)'), THEN('sister (?x) (?y)'))
mother_rule = IF (AND('female (?m)', 'parent (?m) (?x)'), THEN('mother (?m) (?x)'))
father_rule = IF (AND('male (?m)', 'parent (?m) (?x)'), THEN('father (?m) (?x)'))
son_rule = IF (AND('male (?s)', 'parent (?p) (?s)'), THEN('son (?s) (?p)'))
daughter_rule = IF (AND('female (?d)', 'parent (?p) (?d)'), THEN('daughter (?d) (?p)'))
cousing_rule = IF (AND('parent (?p1) (?c1)', 'parent (?p2) (?c2)', 'sibling (?p1) (?p2)'), THEN ('cousin (?c1) (?c2)'))
grandparent_rule = IF (AND('parent (?gp) (?p)', 'parent (?p) (?gc)'), THEN('grandparent (?gp) (?gc)', 'grandchild (?gc) (?gp)'))

# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [same_identity_rule,sibling_rule,brother_rule,sister_rule,
                mother_rule,father_rule,son_rule,daughter_rule,cousing_rule,grandparent_rule,grandparent_rule]
...
WHAT_I_FOUND_INTERESTING = 'all the things!'
WHAT_I_FOUND_BORING = 'took a while to figure out that if '
